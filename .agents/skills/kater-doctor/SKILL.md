---
name: kater-doctor
description: Run Kater doctor diagnostics, fix plans, and safe apply for MCP/Codex wiring.
details: Use for /kater-doctor, "kater doctor", MCP config drift, Codex mcp.json mismatches, or pre-serve environment checks.
---

# Kater doctor

Diagnostics for profiles, adapter secrets, Codex MCP wiring, and public-deploy safety.
...
- After changing profiles, `.kater/.env`, or Codex MCP paths
...
**Inspect a specific Codex MCP file**:
...
- **Codex MCP drift** — prefer `kater up` or doctor apply over hand-editing `.cursor/mcp.json`
