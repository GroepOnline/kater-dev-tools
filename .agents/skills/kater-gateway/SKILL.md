...
| --- | --- | --- |
| MCP SSE | `http://127.0.0.1:9090/sse` | Codex MCP (via `.cursor/mcp.json`) |
...
**Desktop IDE one-shot** — init + write gitignored Codex MCP config + serve:

```bash
uv run kater up
```

`kater up` writes `.cursor/mcp.json` (gitignored). Point Codex at the SSE URL above.
