...
class BrowserSessionManager:
    ...
    def __init__(
        ...
        *,
        ...
        Parameters:
            ...
            policy (BrowserPolicy | None): Session policy; the configured policy is loaded when
                omitted.
    ...
    def act(self, session_id: str, action: BrowserAction) -> ActionResult:
        ...
        Returns:
            ActionResult: The action outcome, including an error result when the session is invalid
                or execution fails.
        """
        started = self._clock()
        with self._lock:
    ...
    def _finish(
        ...
        """
        Finalize an action by updating the session state, persisting its results, and emitting
            telemetry.
