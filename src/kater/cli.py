...
def migrate_apply_command(
    ...
    try:
        ...
        raise typer.Exit(code=1) from exc
    payload = [
        {"version": r.version, "name": r.name, "status": r.status} for r in results
    ]
    if json_output:
...
def browser_sessions_command(
    live: Annotated[
        bool, typer.Option("--live", help="Hide closed/failed sessions.")
    ] = False,
    json_output: Annotated[bool, typer.Option("--json", help="Output as JSON.")] = False,
) -> None:
...
def browser_open_command(
    ...
    if result.get("ok") and url:
        ...
        result["navigate"] = browser_act_tool(
            session_id=session_id, kind="navigate", url=url
        )
    if json_output:
        _print_json(result)
        return
