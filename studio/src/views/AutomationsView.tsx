...
export function AutomationsView() {
  const { data, error, loading, pendingId, refresh, runNow, setEnabled, saveSchedule } = useAutomationsData();
  ...
  return <section className="view-stack">
    ...
    {data?.automations.length ? <div className="automation-list">{data.automations.map(item => <AutomationCard automation={item} pending={pendingId === item.id} onRun={id => void runNow(id)} onToggle={(id, value) => void setEnabled(id, value)} onSaveSchedule={(id, seconds) => void saveSchedule(id, seconds)} key={item.id} />)}</div> : !loading && <EmptyState>No automations configured.</EmptyState>}
  </section>;
}
