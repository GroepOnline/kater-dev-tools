...
class FakeProvider(BrowserProvider):
    ...
    def close_page(self, handle: Any) -> None:
        """Record the session associated with a closed page handle."""
...
class FakeClock:
    def __init__(self, start: float = 1000.0) -> None:
        """Initialize the clock at the specified time."""
        self.now = start
...
class _PageHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        """Serve the page content corresponding to the requested path."""
